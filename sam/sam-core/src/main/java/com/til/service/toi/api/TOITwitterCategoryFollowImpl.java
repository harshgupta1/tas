...
@Service("toiTwitterCategoryFollowImpl")
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class TOITwitterCategoryFollowImpl implements Follow{
	
	private static final Logger log = LoggerFactory.getLogger(TOITwitterCategoryFollowImpl.class);
	...
	@Autowired
	public TOITwitterCategoryFollowImpl(int startPage, int resultsPerThread, Website website)
	{
	...
	public void run() {
		...
		try
		{
					...
					if(topicPageTwitterId != null && !"".equals(topicPageTwitterId))
					{
						...
						if(topicPage.getFeedUrl() != null && !"".equals(topicPage.getFeedUrl()))
						{
								...
								if(feedData != null && !"".equals(feedData))
								{
									...
									catch(UnmarshallingFailureException e)
									{
										try {
											...
											if(i==4)
											{
												log.error("UnmarshallingFailureException in getting RSS data " + feedData + " after " + (i+1) 
									...
									rss2Feeds.getChannel().getItemList().size() > 0 && !topicPage.getPush_all_article())
							{
								...
								// break condition only applicable for twitter case where 1 website will have 1 topic
								...
								if(channelItemList!=null && !channelItemList.isEmpty())
								{
									...
									{
										// This represents the item that is to be tweeted
			...
			if("true".equalsIgnoreCase(post)) {
				log.info("posting is enabled for twitter");
				twitterPage.postArticle(website);
			} else {
				log.warn("posting is disabled for twitter");
			}
		}
		catch(Exception e)
		{
