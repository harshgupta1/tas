...
@Service("toiFacebookCategoryFollowImpl")
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class TOIFacebookCategoryFollowImpl implements Follow{
	
	private static final Logger log = LoggerFactory.getLogger(TOIFacebookCategoryFollowImpl.class);
	...
	@Autowired
	public TOIFacebookCategoryFollowImpl(int startPage, int resultsPerThread, Website website)
	{
					...
					if(topicPage.getFeedUrl() != null && !"".equals(topicPage.getFeedUrl()))
					{
									...
									rss2Feeds.getChannel().getItemList().size() > 0 && !topicPage.getPush_all_article())
						{
							...
							if(item != null)
							{
								...
								{
									String a1 = item.getUrl().substring(item.getUrl().lastIndexOf("/")+1,item.getUrl().lastIndexOf("."));
