...
@Service("toiBlogsTwitterCategoryFollowImpl")
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class TOIBlogsTwitterCategoryFollowImpl implements Follow{
	
	private static final Logger log = LoggerFactory.getLogger(TOIBlogsTwitterCategoryFollowImpl.class);
	...
	@Autowired
	public TOIBlogsTwitterCategoryFollowImpl(int startPage, int resultsPerThread, Website website)
	{
				...
				if(topicPageFacebookId != null && !"".equals(topicPageFacebookId))
				{
					...
					if(topicPage.getFeedUrl() != null && !"".equals(topicPage.getFeedUrl()))
					{
						...
						try {
							...
							feedData = null;
							rss2Feeds = null;
							log.error("IOException getting data from TOI Blogs feed "+topicPage.getUrl(), e);
						} catch (Exception e) {
							feedData = null;
							rss2Feeds = null;
							log.error("Exception getting data from TOI Blogs feed "+topicPage.getUrl(), e); 
						}
						...
						if(feedData != null && !"".equals(feedData))
						{
							...
							{
								rss2Feeds = null;
								log.error("Error parsing TOI Blogs category feed data for url " + topicPage.getFeedUrl() + " feed data \n"+ feedData, e);
							}
						}
									...
									rss2Feeds.getChannel().getItemList().size() > 0 && !topicPage.getPush_all_article())
						{
							...
							try
							{
								...
								dataObject = ToiUtils.parseiBeatLast4HoursFeed(pageViewsJson[1]);
							}
							catch(JSONException e)
							{
								log.error("JSONException getting data from iBeat page views api of url "+url,e);
							}
							catch(Exception e)
							{
							...
							if(dataObject != null)
							{
								for (ChannelItem channelItem : channelItemList) {
									if(channelItem.getPubDate().after(cal.getTime()) )
									{
										try
										{
											JSONObject clicksObject = dataObject.getJSONObject(channelItem.getArticleid());
											_4hourlyClicks = clicksObject.getLong("hourlyClicks");
								...
								topicPage.getPush_all_article())
						{
							...
							for (ChannelItem item : rss2Feeds.getChannel().getItemList()) {
								...
								{
									link = new StringBuffer(item.getLink());
									int lastSlashPos = link.lastIndexOf("/");
									if(lastSlashPos > 0)
									{
										String articleid = Utilities.encodePassword(link.substring(lastSlashPos+1),"MD5");
										for (Article article : articleList) {
											if(article.getArticleid().equalsIgnoreCase(articleid))
											{
												articleFound = true;
												break;
											}
