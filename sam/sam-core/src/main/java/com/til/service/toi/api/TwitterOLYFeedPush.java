...
@Service("twitterOLYFeedPush")
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class TwitterOLYFeedPush implements Follow{
	
	private static final Logger log = LoggerFactory.getLogger(TwitterOLYFeedPush.class);
	...
	@Autowired
	public TwitterOLYFeedPush(int startPage, int resultsPerThread, Website website)
	{
	...
	public void run() {
				...
				if(topicPageTwitterId != null && !"".equals(topicPageTwitterId))
				{
					...
					if(topicPage.getFeedUrl() != null && !"".equals(topicPage.getFeedUrl()))
					{
						...
						try {
							...
							feedData = feedXML[1];
							log.debug("Getting data for Twitter from Olympics feed {} \n {}" ,topicPage.getFeedUrl(),feedData);
						}
						catch (IOException e) {
							feedData = null;
							rss2Feeds = null;
							log.error("IOException getting data for Twitter from Olympics News  feed {} \n {}" ,topicPage.getUrl(), e);
						} catch (Exception e) {
							feedData = null;
							rss2Feeds = null;
							log.error("Exception getting data for Twitter from Olympics News feed {} \n {}" ,topicPage.getUrl(), e); 
						}
						...
						Rss2Feeds rssF = (Rss2Feeds)twitterPage.appendAdditionalFeedsData(topicPage);
		...
		{
			e.printStackTrace();
			log.error("This exception was handled earlier but due to other jobs, it was causing current thread to die. Please check. ", e);
		}
	}
